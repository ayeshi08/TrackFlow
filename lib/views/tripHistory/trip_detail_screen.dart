...
class TripDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final detailsContentBlock = [
      ...
      Row(
        children: [
          _statBox(context, 'Distance', '${trip.distance.toStringAsFixed(2)} km', Icons.straighten),
          const SizedBox(width: 12),
          _statBox(context, 'Duration', _formatDuration(trip.startTime, trip.endTime), Icons.timer),
        ],
      ),
      const SizedBox(height: 12),
      Row(
        children: [
          _statBox(context, 'Avg Speed', '${trip.avgSpeed.toStringAsFixed(1)} km/h', Icons.speed),
          const SizedBox(width: 12),
          _statBox(context, 'GPS Points', '${trip.route.length} recorded', Icons.location_on),
        ],
      ),
  ...
  Widget _statBox(BuildContext context, String title, String value, IconData icon) {
    final theme = Theme.of(context);
    return Expanded(
      child: Container(
        padding: const EdgeInsets.all(12),
